...
export default async function agentRoutes(fastify) {
  ...
  fastify.get('/agent/sessions', async (req, reply) => {
    ...
    let query = supabase.from('sessions').select('*, repos(name, url)').in('repo_id', repoIds).order('created_at', { ascending: false })
  ...
  fastify.post('/agent/start', async (req, reply) => {
    ...
    const owner_id = req.user.id
    if (!repo_id || !task) return reply.status(400).send({ error: 'Missing repo_id or task' })
  ...
  fastify.post('/agent/approve-plan', async (req, reply) => {
    ...
    const { data: session, error } = await supabase.from('sessions').select('*').eq('id', session_id).single()
    ...
    const { data: repo } = await supabase.from('repos').select('owner_id').eq('id', session.repo_id).single()
    if (!repo) return reply.status(404).send({ error: 'Repo not found' })
    
    const planModels = session?.plan || {}
    runCoderLoop({ fastify, supabase, sessionId: session_id, repoId: session.repo_id, ownerId: repo.owner_id, plannerModel: planModels.plannerModel, coderModel: planModels.coderModel })
      .catch(async (err) => {
  ...
  fastify.post('/agent/edit-plan', async (req, reply) => {
    ...
    if (!session_id || !subtasks) return reply.status(400).send({ error: 'Missing session_id or subtasks' })
    
    const { data: session } = await supabase.from('sessions').select('status, plan').eq('id', session_id).single()
    if (!session || session.status !== 'plan_review') return reply.status(400).send({ error: 'Session not in plan_review status' })
    
    await supabase.from('tasks').delete().eq('session_id', session_id).eq('status', 'pending')
    for (const subtask of subtasks) {
      await supabase.from('tasks').insert({ session_id, type: 'WRITE', instruction: subtask.instruction, file_path: normalizePath(subtask.file_path), status: 'pending' })
    }
    if (session.plan) {
  ...
  fastify.get('/agent/session/:id', async (req, reply) => {
    const { id } = req.params
    const { data, error } = await supabase.from('sessions').select(`*, tasks (*), code_drafts (*)`).eq('id', id).single()
    if (error) return reply.status(500).send({ error: error.message })
  ...
  fastify.get('/agent/session/:id/stream-plan', async (req, reply) => {
    ...
    let aborted = false
    req.raw.on('close', () => { aborted = true }) // ✅ FIX: Prevent memory leak on disconnect
  ...
  fastify.get('/agent/task/:id/stream-code', async (req, reply) => {
    ...
    const maxAttempts = 120
    while (attempts < maxAttempts && !aborted) {
      const { data: drafts } = await supabase.from('code_drafts').select('new_content, explanation').eq('task_id', id).order('created_at', { ascending: false }).limit(1)
  ...
  fastify.post('/agent/approve', async (req, reply) => {
    ...
    const { data: draft, error: draftError } = await supabase.from('code_drafts').select('*, tasks(*), sessions(*)').eq('id', draft_id).single()
    ...
    if (!pat || !repo) return reply.status(500).send({ error: 'Missing PAT or repo identifier' })
  ...
  fastify.post('/agent/feedback', async (req, reply) => {
    ...
    const owner_id = req.user.id
    if (!draft_id || !feedback) return reply.status(400).send({ error: 'Missing draft_id or feedback' })
...
export async function runAgentLoop({ fastify, supabase, sessionId, repoId, ownerId, task, plannerModel: passedPlannerModel, coderModel: passedCoderModel }) {
  ...
  for (const subtask of plan.subtasks) {
    ...
    await supabase.from('tasks').insert({ session_id: sessionId, type: 'WRITE', instruction: subtask.instruction, file_path: normalizePath(subtask.file_path), status: 'pending' })
  }
...
export async function runCoderLoop({ fastify, supabase, sessionId, repoId, ownerId, plannerModel: passedPlannerModel, coderModel: passedCoderModel }) {
  try {
    ...
    const { data: tasks } = await supabase.from('tasks').select('*').eq('session_id', sessionId).eq('status', 'pending').order('created_at', { ascending: true })
    ...
    for (const task of tasks) {
      try {
        ...
        const newContent = await runCoder({ filePath, language, currentContent: fileContent, instruction: task.instruction, risk: subtaskMeta.risk, riskReason: subtaskMeta.risk_reason, coderModel, apiKey })
        const explanation = await generateExplanation({ instruction: task.instruction, originalContent: fileContent, newContent, plannerModel, apiKey })
        
        await supabase.from('code_drafts').insert({ session_id: sessionId, task_id: task.id, file_path: filePath, original_content: fileContent, new_content: newContent, explanation, verdict: 'awaiting_approval' })
...
async function runFeedbackLoop({ fastify, supabase, draft, feedback, repoId, sessionId, ownerId, originalTask, plannerModel: passedPlannerModel, coderModel: passedCoderModel }) {
  try {
    // ✅ FIX: Prevent infinite credit-draining feedback loops
    const currentRetries = draft.tasks?.retries || 0
    if (currentRetries >= 3) {
    ...
    const replan = await replanSubtask({ originalTask, originalInstruction: draft.tasks?.instruction, failedCode: draft.new_content, feedback, context, memory, plannerModel, apiKey })
    
    await supabase.from('tasks').update({ instruction: replan.subtask.instruction, status: 'running', retries: currentRetries + 1 }).eq('id', draft.task_id)
    ...
    const newContent = await runCoder({ filePath, language, currentContent: fileContent, instruction: replan.subtask.instruction, risk: replan.subtask.risk, riskReason: replan.subtask.risk_reason, feedback, coderModel, apiKey })
    const explanation = await generateExplanation({ instruction: replan.subtask.instruction, originalContent: fileContent, newContent, plannerModel, apiKey })
    
    await supabase.from('code_drafts').insert({ session_id: sessionId, task_id: draft.task_id, file_path: filePath, original_content: fileContent, new_content: newContent, explanation, verdict: 'awaiting_approval' })
