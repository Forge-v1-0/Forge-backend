...
if (!process.env.ENCRYPTION_KEY || process.env.ENCRYPTION_KEY.length < 32) {
  console.error(
    'FATAL: ENCRYPTION_KEY env var is missing or shorter than 32 chars. Set it and restart.'
  )
  process.exit(1)
}
...
// Strict regex to ensure the stored value is exactly:
// 32 hex chars, a colon, then ciphertext hex
...
export function decrypt(stored) {
  // Catch nulls/undefined immediately
  if (typeof stored !== 'string') {
    throw new Error(
      'decrypt: stored value is not a string — credential may be missing or corrupt'
    )
  }

  // Catch legacy plaintext values
